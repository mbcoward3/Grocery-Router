...
`Source` columns of `corpus.md` and `candidates.md`. No key, no dependency, and
no way to surface a page that does not exist.

**How it searches is in `acquire/adapters.py`**, and it is an interface with five
implementations rather than one endpoint. They run from *the site told us how* to
*we worked it out* - the WordPress REST routes, then the site's own published
`SearchAction`, then its search page, then its sitemap - and the first that
answers for a host is remembered. A source that needs something new is a class
there, not an edit here. `robots.txt` is obeyed and requests to one host are
spaced out.

That ladder is safe to have because of where the invention bar actually sits:
**an adapter only ever proposes URLs.** Nothing in it parses a recipe.
`onboard.from_url` then refuses any page without machine-readable schema.org
data, so the loosest strategy in the file can cost a wasted request and can never
cost a wrong ingredient.
...
# Kept for callers that still import them from here. The courtesy delay itself
# lives in `adapters`, where it is per host rather than per loop iteration.
PAUSE = 0.0
...
MAX_FETCHES = 12
# The two scraping strategies - the site's own search page and its sitemap - are
# on by default and switchable off in one place. Off, only the routes a site
# published for programs are used, which is the stricter posture and the right
# one to be able to fall back to without editing code.
ALLOW_SCRAPING = True
...
def search(query: str, host: str, limit: int = MAX_HITS_PER_SOURCE,
           log=lambda *_: None) -> list[dict]:
    """Search one source. Returns `[{title, url, host}]`.

    A thin pass-through to `adapters.search`, kept as a name here because it is
    the seam every test stubs and because the pipeline should not care which of
    the five strategies answered.
    """
    return adapters.search(query, host, limit, allow_scraping=ALLOW_SCRAPING,
                           log=log)
...
def look(gap: Gap, hosts: list[str], known: set[str], budget: list[int],
    ...
    for host in hosts:
        if budget[0] <= 0:
            break
        try:
            hits = search(gap.query, host, log=log)
