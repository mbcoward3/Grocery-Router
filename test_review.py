...
class Logged(unittest.TestCase):
    ...
    def propose(self, week, *, added, planner="ranker", **extra):
        ...
        pantry.log(self.hh, "proposed", week=week, nights=len(added), guests=0, risk="normal",
    ...
    def drop(self, week, slug, kind):
        pantry.log(self.hh, "drop", week=week, recipe=slug, reason_shown="x",
                   reason_kind=kind, candidate=False)

    def kept(self, week, slug):
        pantry.log(self.hh, "feedback_applied", recipe=slug, outcome="kept", by="Sam", week=week)
...
class TestWhichReasonsLand(Logged):
    def test_a_reason_that_gets_dropped_scores_lower_than_one_that_does_not(self):
        ...
        by_kind = {r["kind"]: r for r in review.reasons(self.hh)}
    ...
    def test_worst_first(self):
        self.propose("2026-06-01", added=[("a", "stale", False, "beef"),
                                          ("b", "plain", False, "pork")])
        self.drop("2026-06-01", "b", "plain")
        self.assertEqual(review.reasons(self.hh)[0]["kind"], "plain")
    ...
    def test_dropped_and_kept_are_different_denominators(self):
        ...
        row = {r["kind"]: r for r in review.reasons(self.hh)}["stale"]
    ...
    def test_the_same_recipe_twice_is_two_offers(self):
        ...
        self.assertEqual({r["kind"]: r for r in review.reasons(self.hh)}["stale"]["offered"], 2)
    ...
    def test_a_drop_with_no_recorded_kind_is_joined_back_to_its_proposal(self):
        ...
        pantry.log(self.hh, "drop", week="2026-06-01", recipe="a", reason_shown="x")
        self.assertEqual({r["kind"]: r for r in review.reasons(self.hh)}["yield"]["dropped"], 1)

    def test_an_empty_log_is_empty_not_broken(self):
        self.assertEqual(review.reasons(self.hh), [])
        self.assertEqual(review.breadth(self.hh), [])
        self.assertEqual(review.summary(self.hh)["distinct_recipes"], 0)
...
class TestBreadth(Logged):
    ...
    def test_a_widening_repertoire_shows_as_widening(self):
        ...
        self.assertEqual([r["distinct_so_far"] for r in review.breadth(self.hh)], [1, 2, 3])
    ...
    def test_it_groups_by_the_week_planned_not_the_day_it_ran(self):
        ...
        self.propose("2026-06-01", added=[("a", "stale", False, "beef")])
        self.propose("2026-06-08", added=[("b", "stale", False, "pork")])
        self.assertEqual([r["week"] for r in review.breadth(self.hh)],
    ...
    def test_protein_and_cuisine_spread_are_counted(self):
        self.propose("2026-06-01", added=[("a", "stale", False, "beef"),
                                          ("b", "stale", False, "pork"),
                                          ("c", "stale", False, "beef")])
        self.assertEqual(review.breadth(self.hh)[0]["proteins"], 2)
...
class TestThePlannerComparison(Logged):
    def test_the_two_implementations_are_compared_on_real_weeks(self):
        ...
        self.drop("2026-06-01", "b", "plain")
        by = {r["planner"]: r for r in review.planners(self.hh)["by_planner"]}
    ...
    def test_no_model_week_yet_is_itself_the_answer(self):
        self.propose("2026-06-01", added=[("a", "stale", False, "beef")])
        by = {r["planner"] for r in review.planners(self.hh)["by_planner"]}
        self.assertEqual(by, {"ranker"})


class TestAcquisitionShowsUp(Logged):
    def test_searching_and_pasting_are_told_apart(self):
        pantry.log(self.hh, "acquired", recipe="a", source="https://x.test/a/", found_by="acquire")
        pantry.log(self.hh, "acquired", recipe="b", source="https://y.test/b/", found_by="paste")
        out = review.acquisition(self.hh)
        self.assertEqual(out["total"], 2)
        self.assertEqual(out["by_route"], {"acquire": 1, "paste": 1})

    def test_nothing_acquired_has_been_proven_yet(self):
        pantry.log(self.hh, "acquired", recipe="a", source="https://x.test/a/", found_by="acquire")
        self.assertEqual(review.acquisition(self.hh)["promoted"], 0)
...
class TestTheLogStaysTrue(Logged):
    def test_a_payload_may_not_overwrite_the_decision_type(self):
        ...
        with self.assertRaises(ValueError):
            pantry.log(self.hh, "drop", kind="stale")
        with self.assertRaises(ValueError):
            pantry.log(self.hh, "drop", at="whenever")

    def test_the_kind_a_meal_was_offered_under_is_recoverable(self):
        self.propose("2026-06-01", added=[("chili", "stale", False, "beef")])
        self.assertEqual(review.kind_of(self.hh, "chili"), "stale")
        self.assertEqual(review.kind_of(self.hh, "never-offered"), "")

    def test_the_most_recent_offer_wins(self):
        self.propose("2026-06-01", added=[("chili", "stale", False, "beef")])
        self.propose("2026-06-08", added=[("chili", "protein", False, "beef")])
        self.assertEqual(review.kind_of(self.hh, "chili"), "protein")
...
class TestTheRankerRecordsItsKinds(Logged):
    ...
    def test_a_real_proposal_carries_reason_kinds(self):
        pantry.propose(self.hh, 5, planner="ranker", week="2026-06-01")
        kinds = {o["kind"] for o in review.offers(self.hh)}
        self.assertTrue(kinds)
        self.assertNotIn("", kinds)

    def test_the_kinds_are_the_ones_review_can_explain(self):
        pantry.propose(self.hh, 5, planner="ranker", week="2026-06-01")
        for offer in review.offers(self.hh):
