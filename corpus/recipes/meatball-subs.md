---
format_version: 1
key: meatball-subs
name: Meatball Subs
status: verified
approved_on: '2026-08-17'
source:
  relationship: source
  url: https://www.spendwithpennies.com/meatball-sub/
  attribution: Spend With Pennies — Meatball Subs
  checked_on: '2026-08-17'
yield: 4 servings
hands_on:
  min: 20
  max: 20
unattended:
  min: 35
  max: 35
ingredient_sections:
- name: Meatballs
  ingredients:
  - source_text: 1 pound lean ground beef
    grocery_item:
      key: lean-ground-beef
      name: Lean Ground Beef
      store_section:
        key: meat
        name: Meat
      shopping_mode: measured
    quantity:
      kind: exact
      amount: '1'
      unit: lb
  - source_text: 1 egg
    grocery_item:
      key: eggs
      name: Eggs
      store_section:
        key: dairy
        name: Dairy
      shopping_mode: counted
    quantity:
      kind: exact
      amount: '1'
      unit: each
  - source_text: ½ cup seasoned bread crumbs
    grocery_item:
      key: seasoned-breadcrumbs
      name: Seasoned Breadcrumbs
      store_section:
        key: baking
        name: Baking
      shopping_mode: measured
    quantity:
      kind: exact
      amount: 1/2
      unit: cup
  - source_text: ½ teaspoon Italian seasoning
    grocery_item:
      key: italian-seasoning
      name: Italian Seasoning
      store_section:
        key: spices
        name: Spices
      shopping_mode: measured
    quantity:
      kind: exact
      amount: 1/2
      unit: tsp
  - source_text: ½ teaspoon onion powder
    grocery_item:
      key: onion-powder
      name: Onion Powder
      store_section:
        key: spices
        name: Spices
      shopping_mode: measured
    quantity:
      kind: exact
      amount: 1/2
      unit: tsp
  - source_text: salt and black pepper to taste
    grocery_item:
      key: salt
      name: Salt
      store_section:
        key: spices
        name: Spices
      shopping_mode: measured
    quantity:
      kind: unspecified
  - source_text: salt and black pepper to taste
    grocery_item:
      key: black-pepper
      name: Black Pepper
      store_section:
        key: spices
        name: Spices
      shopping_mode: measured
    quantity:
      kind: unspecified
- name: Subs
  ingredients:
  - source_text: 2 cups pasta sauce or marinara sauce
    grocery_item:
      key: marinara-sauce
      name: Marinara Sauce
      store_section:
        key: pantry
        name: Pantry
      shopping_mode: counted
    quantity:
      kind: exact
      amount: '2'
      unit: cup
  - source_text: 4 hoagie rolls
    grocery_item:
      key: hoagie-rolls
      name: Hoagie Rolls
      store_section:
        key: bakery
        name: Bakery
      shopping_mode: counted
    quantity:
      kind: exact
      amount: '4'
      unit: each
  - source_text: 4 tablespoons garlic butter
    grocery_item:
      key: garlic-butter
      name: Garlic Butter
      store_section:
        key: dairy
        name: Dairy
      shopping_mode: measured
    quantity:
      kind: exact
      amount: '4'
      unit: tbsp
  - source_text: 1 cup shredded Italian cheese blend or mozzarella or provolone
    grocery_item:
      key: shredded-italian-cheese-blend
      name: Shredded Italian Cheese Blend
      store_section:
        key: dairy
        name: Dairy
      shopping_mode: measured
    quantity:
      kind: exact
      amount: '1'
      unit: cup
instruction_sections:
- name: Method
  steps:
  - Preheat the oven to 350°F. Combine the meatball ingredients, shape into 1-inch balls, place on a parchment-lined pan, and bake for 20 minutes or until cooked through. For a shortcut, 16 frozen meatballs can be prepared according to their package directions instead.
  - Simmer the cooked meatballs in the marinara sauce, covered, for 10 minutes.
  - Spread garlic butter inside each roll and broil for about 3 minutes, until golden.
  - Place four meatballs in each roll, add sauce and cheese, and broil for 2 to 3 minutes until melted. Serve with additional sauce if desired.
review:
- field: source_and_alternatives
  kind: conflict-resolved
  note: Recovered the current authoritative recipe, which now specifies homemade meatballs. Household re-review kept homemade meatballs and the primary shredded Italian cheese blend, with a non-binding suggestion that 16 frozen meatballs can be used as a shortcut.
  approved: true
- field: instruction_sections
  kind: rewritten
  note: Condensed the current authoritative method while preserving all timed cooking stages and contributions.
  approved: true
---

# Meatball Subs

> Approved bootstrap recipe. YAML front matter is ingested into SQLite; the sections below
> are the checked human-readable view.

## Recipe details

- Source: [Spend With Pennies — Meatball Subs](https://www.spendwithpennies.com/meatball-sub/) (`source`)
- Source checked: 2026-08-17
- Yield: 4 servings
- Hands-on: 20 minutes
- Unattended: 35 minutes

## Ingredients

### Meatballs

- 1 pound lean ground beef
  - Shopping: 1 lb Lean Ground Beef — Meat
- 1 egg
  - Shopping: 1 Eggs — Dairy
- ½ cup seasoned bread crumbs
  - Shopping: 1/2 cup Seasoned Breadcrumbs — Baking
- ½ teaspoon Italian seasoning
  - Shopping: 1/2 tsp Italian Seasoning — Spices
- ½ teaspoon onion powder
  - Shopping: 1/2 tsp Onion Powder — Spices
- salt and black pepper to taste
  - Shopping: Salt — Spices
- salt and black pepper to taste
  - Shopping: Black Pepper — Spices

### Subs

- 2 cups pasta sauce or marinara sauce
  - Shopping: 2 cup Marinara Sauce — Pantry
- 4 hoagie rolls
  - Shopping: 4 Hoagie Rolls — Bakery
- 4 tablespoons garlic butter
  - Shopping: 4 tbsp Garlic Butter — Dairy
- 1 cup shredded Italian cheese blend or mozzarella or provolone
  - Shopping: 1 cup Shredded Italian Cheese Blend — Dairy

## Instructions

### Method

1. Preheat the oven to 350°F. Combine the meatball ingredients, shape into 1-inch balls, place on a parchment-lined pan, and bake for 20 minutes or until cooked through. For a shortcut, 16 frozen meatballs can be prepared according to their package directions instead.
2. Simmer the cooked meatballs in the marinara sauce, covered, for 10 minutes.
3. Spread garlic butter inside each roll and broil for about 3 minutes, until golden.
4. Place four meatballs in each roll, add sauce and cheese, and broil for 2 to 3 minutes until melted. Serve with additional sauce if desired.

## One-batch grocery preview

### Bakery

- 4 Hoagie Rolls

### Baking

- 1/2 cup Seasoned Breadcrumbs

### Dairy

- 1 Eggs
- 4 tbsp Garlic Butter
- 1 cup Shredded Italian Cheese Blend

### Meat

- 1 lb Lean Ground Beef

### Pantry

- 2 cup Marinara Sauce

### Spices

- 1/2 tsp Italian Seasoning
- 1/2 tsp Onion Powder
- Salt
- Black Pepper

## Approved true-up decisions

- `source_and_alternatives` — **conflict-resolved:** Recovered the current authoritative recipe, which now specifies homemade meatballs. Household re-review kept homemade meatballs and the primary shredded Italian cheese blend, with a non-binding suggestion that 16 frozen meatballs can be used as a shortcut.
- `instruction_sections` — **rewritten:** Condensed the current authoritative method while preserving all timed cooking stages and contributions.
